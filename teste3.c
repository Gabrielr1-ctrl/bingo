...
int main(){
	...
	if(entrada == 1){
        ...
        //GERANDO CARTELA 1
		int matriz[4][6] = {{0}};
    int aux[25]; // vetor auxiliar para armazenar os números gerados
    int i, j, num, found,k;
    ...
    // gerar números aleatórios e preencher a matriz
    srand(time(NULL)); // inicializar o gerador de números aleatórios
    for (i = 0; i < 5; i++) {
        for (j = 0; j < 5; j++) {
            do {
                num = rand() % 99 + 1; // gerar um número aleatório entre 1 e 99
                found = 0;
                for ( k = 0; k < 25; k++) {
                    if (aux[k] == num) {
                        found = 1;
            ...
            matriz[i][j] = num;
            aux[i * 5 + j] = num; // armazenar o número no vetor auxiliar
        }
    }
    for (i = 0; i < 5; i++) {
        for (j = 0; j < 5; j++) {
